 ...
 *
 * Interview Note:
 * This class demonstrates JPA entity design with:
 * - UUID primary key (better for distributed systems)
 * - LOB (Large Object) fields for storing LaTeX/PDF content
 * - Embedded objects for complex nested data
 ...
 *
 * Interview Note:
 * Using H2 in-memory database for simplicity. In production,
 * PDF bytes might be stored in object storage (S3) with only
 * the reference stored in the database.
 */
@Entity
...
public class GeneratedCv {
     ...
     *
     * Interview Note:
     * UUID vs Long ID: UUID is better for security and distribution,
     * Long is better for performance and simplicity.
     */
    @Id
     ...
     *
     * Interview Note:
     * CLOB is used for text larger than typical VARCHAR limits (usually 4000 chars).
     * LaTeX CVs can easily exceed this limit.
     */
     ...
     *
     * Interview Note:
     * In production, consider storing PDFs in object storage (S3, Azure Blob)
     * and keeping only the URL in the database for better scalability.
     */
     ...
     *
     * Interview Note:
     * Storing JSON in a text column is simple but not ideal for querying.
     * For complex queries on coach_brief, consider using JSON column type
     * (available in PostgreSQL) or a separate entity structure.
     */
     ...
     *
     * Interview Note:
     * @PrePersist is a JPA lifecycle callback that runs before the entity
     * is inserted into the database. Perfect for setting default values.
     */
    @PrePersist
